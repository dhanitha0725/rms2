...
namespace Application.Features.ManageReservations.AddReservationByCustomer.CompleteReservation
{
    public class MakeReservationCommandHandler(
        ...
        ILogger logger)
        : IRequestHandler<MakeReservationCommand, Result<int>>
    {
            ...
            try
            {
                ...
                if (request.AuthenticatedUserRole == "Employee" && dto is { GuestEmail: not null, GuestPhone: not null })
                {
                    ...
                    if (!guestResult.IsSuccess)
                    {
                        ...
                        return Result<int>.Failure(new Error("Failed to add guest."));
                    }
                ...
                else
                {
                    ...
                    return Result<int>.Failure(new Error("Invalid user role or missing guest details"));
                }
                ...
                if (!availabilityResult.IsSuccess)
                {
                    ...
                    return Result<int>.Failure(new Error("Error with checking availability."));
                }
                ...
                if (!priceResult.IsSuccess)
                {
                    ...
                    return Result<int>.Failure(new Error("Error with calculating price."));
                }
                ...
                if (!reservationResult.IsSuccess)
                {
                    ...
                    return Result<int>.Failure(new Error("Error with creating reservation."));
                }
                ...
                if (dto.CustomerType != "Private" && dto.Documents?.Count > 0)
                {
                    ...
                    if (!uploadResult.IsSuccess)
                    {
                        ...
                        return Result<int>.Failure(new Error("Error with uploading documents."));
                    }
                }
                ...
                if (dto.CustomerType == "Private" || dto.AmountPaid > 0)
                {
                    ...
                    if (!paymentResult.IsSuccess)
                    {
                        ...
                        return Result<int>.Failure(new Error("Error with processing payment."));
                    }
                }
                ...
                return Result<int>.Success(reservationId);
            }
            catch (Exception e)
            {
